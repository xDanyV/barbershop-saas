...
export default function SlotCard({ time, selectedDate, barberId, onBook }: Props) {
  ...
  // Toggle popover and detect if it should open upward
  const handleToggle = () => {
  ...
  // Handle confirm button click in popover
  ...
  return (
    <>
      <div className="relative">
        {/* Slot row */}
        <div
          className={`flex items-center justify-between border rounded-xl px-4 py-3 transition ${popoverOpen
        ...
        {popoverOpen && (
          <div
            ref={popoverRef}
            className={`absolute right-0 z-50 w-72 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 animate-in fade-in duration-150
              ${openUpward ? "bottom-full mb-2" : "top-full mt-2"}
            `}
          >
            {/* Arrow */}
            <div
              className={`absolute right-6 w-4 h-4 bg-white border-gray-200 rotate-45 ${openUpward
              ...
              {loading ? (
                ...
                services.map((s) => (
                  <button
                    ...
                    className={`flex items-center justify-between px-3 py-2.5 rounded-xl border text-sm transition cursor-pointer ${selectedService === s.id
